...
<body>
    ...
    // Write the error next to the form input if validation fails.
    $fullname_error = '';
    
    if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['fullname'])) {
        $fullname = trim($_GET['fullname']);
        if (empty($fullname)) {
            $fullname_error = 'Hiba: A munkavállaló neve megadása kötelező!';
        } elseif (str_word_count($fullname) < 2) {
            $fullname_error = 'Hiba: A munkavállaló neve legalább két szóból álljon!';
        }
    }
